---
name: improve
description: Audit a WHOLE codebase as a senior advisor and write self-contained implementation plan FILES for other sessions to execute. Strictly read-only - never implements or refactors anything. Use ONLY when the user explicitly types /improve or explicitly asks for a full-repo audit producing plan files. Do NOT use for ordinary code review, a diff/branch/PR review, cleanup, simplification, security review of pending changes, or anything answerable inline - /code-review, /simplify and /security-review own those.
license: MIT
...
1. **Never modify source code yourself.** No edits, no fixes, no "quick wins while you're in there." The ONLY files you may create or modify live under `plans/` in the repo root — or under `advisor-plans/` when `plans/` already exists for an unrelated purpose (create the chosen directory if absent). The `execute` variant is DISABLED here: never dispatch an executor subagent, and never merge, push, or commit. `plans/` is git-excluded locally.
2. **Never run commands that mutate the user's working tree** — no installs, no builds that write artifacts outside standard ignored dirs, no git commits, no formatters. Read, search, and run read-only analysis only (e.g. `tsc --noEmit`, lint in check mode, `npm audit` / `pnpm audit`, test suite if cheap and side-effect free). No exceptions here - `execute` and `--issues` are both disabled.
...
5. **If the user asks you to implement directly, decline and point at the plan** — offer to refine the plan, or to hand the plan file to a separate session, instead.
...
7. **Never leave AI traces in anything you write.** No "generated by", no tool or assistant names, no bot signatures, no `Co-authored-by:` trailers - not in plan files, `plans/README.md`, branch names, commit messages, or code comments. Do not create or switch branches. Work on the current branch. If the work needs its own branch, print the `git switch -c` command and stop. Take message style from `git log`. Everything reads as the user's own work.
...
- a finish line: the scope and breadth that count as done for that subagent. Its reply ends with a line naming what it did not read.

Audit depth follows the **effort level** (default `quick` here - a full nine-category fan-out rarely pays on a repo this size; raise it with a `standard` / `deep` keyword anywhere in the invocation):

| | `quick` (default) | `standard` | `deep` |
|---|---|---|---|
...
The stops are the selection below, the questions `plan` still has to ask, and the offer `branch` makes to run a full audit. Run every other phase straight through. Write the vetted findings to `plans/findings.md` before asking, so the selection can happen in a later session or another tool.
...
Finish by writing `plans/README.md` with the recommended execution order, dependencies between plans, and a status column. The executor keeps its own row current: `IN PROGRESS — steps 1–N verified`, `BLOCKED — <reason>`, or `DONE`. On resume, start at the first step that row does not list as verified. `DONE` means every done criterion was run. The plan ends with **Needs you / Changed / Found / Not verified**.

## Invocation variants

- Bare invocation → full workflow above.
- `quick` / `deep` (anywhere in the invocation) → effort level for the audit; see the table in Phase 2. Composes with everything: `quick security`, `deep tests`. Default is `quick`.
- With a focus argument (e.g. `security`, `perf`, `tests`) → run Recon, then audit only that category, then plan.
- `branch` → audit only the current working branch's changes: scope = files changed since the merge-base with the default branch (`git diff --name-only $(git merge-base origin/<default> HEAD)..HEAD`) plus their direct importers/callers. Light recon, all categories, usually no subagents. **Tag every finding `introduced` (by this branch) or `pre-existing` (in touched files)** — the table separates them; don't blame the branch for legacy debt, but do surface what it's building on top of. Tag findings that would hold the PR as `blocks merge` and list those first. If on the default branch or zero commits ahead, say so and offer a full audit instead.
